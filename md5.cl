...
__kernel void calculateMd5Hashes(global const password_t* messages, global password_hash_t* out) {
  ...
  uint result[4];
  md5(message.buffer, message.size, result);


  for (i = 0; i < 4; i++) {
    outhash->v[i] = result[i];
  }
}
